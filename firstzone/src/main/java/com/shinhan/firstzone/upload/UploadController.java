...
//        return new ResponseEntity<>(resultDTOList, HttpStatus.OK);
        //-----------------------DB에 저장(하나의 멤버가 여러개의 프로필 이용)
        ProfileEntity entity = ProfileEntity.builder()
        		.fname(saveName).member(member).currentYn(i==uploadFiles.length-1?true:false)
        		.build();
        profileRepo.save(entity);
        i++;
        //ㅜㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡㅡ
    ...
    private String makeFolder() {
        ...
        String folderPath =  str.replace("//", File.separator); //File.separator 파일의 구분자로 변셩
        // make folder --------
        File uploadPathFolder = new File(uploadPath, folderPath);//c:/upload/2024/07/29
